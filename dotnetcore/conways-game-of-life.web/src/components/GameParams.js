...
export default class GameParams extends React.Component {
  ...
  componentDidMount() {
    axios.get(`https://localhost:5001/api/generators`).then(res => {
      const generators = res.data.map(obj => obj);
      this.setState({ generators: generators });
    });

    axios.get(`https://localhost:5001/api/fillfactor`).then(res => {
  ...
  render() {
    return (
      <Form componentClass="form-horozontal">
        <h2>
          Game Parameters <small>Enter game values here</small>
        </h2>
